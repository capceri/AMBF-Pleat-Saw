...
function initTouchKeypad() {
    ...
    keypadButtons.forEach(button => {
        button.addEventListener('click', () => handleNumpadDigit(button.getAttribute('data-num')));
    });

    if (keypadClear) {
        keypadClear.addEventListener('click', handleNumpadClear);
    }

    if (keypadEnter) {
        keypadEnter.addEventListener('click', commitNumpadValue);
    }

    if (keypadCancel) {
        keypadCancel.addEventListener('click', closeInputKeypad);
    }
...
function openInputKeypad(input) {
    ...
    keypadValue = input.value ? input.value.toString() : '0';
    updateNumpadDisplay();
