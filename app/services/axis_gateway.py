...
class AxisGateway:
    ...
    def __init__(
        ...
        io_poller=None,  # legacy signature compatibility (unused)
    ):
        """
    ...
    def set_m3_offset(self, offset_mm: float) -> bool:
        """
        Set engineering offset applied to M3 positions (UI reference).

        Args:
            offset_mm: Non-negative offset in millimetres

        Returns:
            True if applied, False if value invalid
        """
        if offset_mm < 0:
            logger.warning("M3 offset must be non-negative (got %.3f)", offset_mm)
            return False

        with self._lock:
            self.m3_offset_mm = float(offset_mm)

        logger.info("M3 offset set to %.3f mm", self.m3_offset_mm)
        return True

    def get_m3_offset(self) -> float:
        """Return the current engineering offset for M3 (in mm)."""
        return getattr(self, 'm3_offset_mm', 0.0)
