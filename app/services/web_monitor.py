...
class WebMonitor:
    ...
    def _get_motor_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Overlay M2 sensor states from I/O module
            if m2_status is None:
                m2_status = {}
            if self.io:
                try:
                    m2_status['at_s2'] = bool(self.io.get_input('sensor2'))
                    m2_status['at_s3'] = bool(self.io.get_input('sensor3'))
                except Exception:
                    pass
            ...
            m3_backstop['position_mm'] = position_mm if position_mm is not None else 0
    ...
    def _execute_command(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if command == 'start_cycle':
                ...
                if self.axis:
                    self.axis.m3_goto(position, velocity, accel)
                    return {'success': True, 'message': f'M3 moving to {position} mm'}
                return {'success': False, 'error': 'Axis gateway not available'}
