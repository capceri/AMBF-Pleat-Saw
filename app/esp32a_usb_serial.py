...
class ESP32AUSBSerial:
    ...
    def _parse_status(line: str) -> ESP32AStatus:
        ...
        try:
            ...
            if len(parts) >= 2:
                m2_part = parts[1]
                status.m2_in_motion = "MOVING" in m2_part or "FWD" in m2_part or "REV" in m2_part
                if "vel=" in m2_part:
    ...
    def set_m2_velocity(self, vel_mm_s: float) -> Tuple[bool, str]:
        ...
        logger.info(f"ESP32-A: Setting M2 velocity to {vel:.1f} mm/s")
        result = self._send_command(f"2v{vel:.1f}")
        logger.info(f"ESP32-A: M2 velocity response: {result}")
        return result

    def feed_forward(self) -> Tuple[bool, str]:
        logger.info("ESP32-A: Sending M2 feed forward command (2f)")
        result = self._send_command("2f", expect=("M2_FWD",))
        logger.info(f"ESP32-A: M2 feed forward response: {result}")
        return result

    def feed_reverse(self) -> Tuple[bool, str]:
        logger.info("ESP32-A: Sending M2 feed reverse command (2b)")
        result = self._send_command("2b", expect=("M2_REV",))
        logger.info(f"ESP32-A: M2 feed reverse response: {result}")
        return result
    ...
    def query_status(self) -> ESP32AStatus:
        self._send_command("?", timeout=self.status_timeout_s)
        return self.status
